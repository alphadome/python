...
#print(get_noaa_dict("UKM00003772", "T"))
...
def find_nearest_ecad_station(address, weather_type):
    ...
    return eligible_dict   
#print(find_nearest_ecad_station("Iowa","T"))
#print(find_nearest_noaa_station("Iowa","T"))
...
def find_nearest_station(address, weather_type, update=''):
    ...
    def proceed_with_method():    
        ...
        return [code, best_location, database, weather_type]
...
print(find_nearest_station("London","T", 'update'))
#print(find_nearest_station("Iowa","T"))
  

def get_ecad_file_location(address, weather_type, update=''):

    def proceed_with_method():
        station = find_nearest_station(address, weather_type)
        station_code = station[0] 
    ...
    title = "get_ecad_file_location_" + str(weather_type)
    if weather_profile(address, title) == None or update != '':
        print("/t-Adding/refreshing data...")
        data = proceed_with_method()
        weather_profile(address, title, data)
...
def get_ecad_dict(address, weather_type, update = ''):
    
    station = get_ecad_file_location(address, weather_type, update ='')
    filename = station[0]
...
def historic_weather_data(address, weather_type, update=''):
    ...
    def proceed_with_method():
        station_info = find_nearest_station(address, weather_type)
        ...
        if database == "NOAA":
            ...
            weather_dict = get_ecad_dict(address, weather_type, update='')
