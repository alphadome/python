...
def get_noaa_dict(station_code, weather_type):
    ...
    weather_dict = {}
    for line in data:
        if line[1] in weather_dict.keys():
            local_dict = weather_dict[line[1]]
        else:
            local_dict = {}
        ...
        for i in range(2,len(line)):
            ...
            try:
                ...
                if int(item) != int(-999.9):
                    local_dict[weather_date] = float("%0.1f" % item)
            except Exception:
                None
        
        weather_dict[line[1]] = local_dict
    
    
    if weather_type == "T":
        tavg_l = len(weather_dict['TAVG'].keys())
        tmin_l = len(weather_dict['TMIN'].keys())
        tmax_l = len(weather_dict['TMAX'].keys())
        most_entries = max([tavg_l, tmin_l, tmax_l])
        
        if tavg_l == most_entries:
            desired_dict = weather_dict['TAVG']
        elif tavg_l == most_entries:
            desired_dict = weather_dict['TMAX']
        else:
            desired_dict = weather_dict['TMIN']        
        
    elif weather_type == "P":
        desired_dict = weather_dict['PRCP']
    else:
        desired_dict = {}
